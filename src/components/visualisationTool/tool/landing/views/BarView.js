...
const BarView = ({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (document.getElementById("legends-2")) {
        document.getElementById("legends-2").checked = true;
      }
    }, 500);
  ...
  const addColumn = (e) => {
    ...
    let colors = barPalette("cb-Custom1", data.length).map(function (hex) {
      return "#" + hex;
    });
  ...
  const updateLegends = (e) => {
    let value = document.getElementById("legends-2");
  ...
  const updateGrids = (e) => {
    let value = document.getElementById("grid-2");
